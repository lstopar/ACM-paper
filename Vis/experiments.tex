...
Finally in Figure \ref{fig:traffic-finest} the daily cycle starts becoming less apparent.
...
\begin{figure}[h!]
	...
	\caption{Qualitative representation of a traffic counter positioned on the highway ring around Ljubljana. On a high level, we were able to identify what we believe are the typical daily states on the ring with the night state, with low traffic, on the left, the midday state with the highest traffic on the right and a morning and evening states with moderate traffic on the top and bottom. \dunja{see if the font in Fig. 11 can be larger}}
