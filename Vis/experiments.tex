The system is used primarily to identify certain patterns in different types of multivariate time-series. In this section, we highlight several examples - the first from synthetically generated data followed by two real world datasets. Finally we describe some other examples of the use of the system.  These datasets can be found on the StreamStory website (\url{http://streamstory.ijs.si}). 

\subsection{Prediction Evaluation}

This first experiment is intended to test the validity of our model. The model was constructed on simulated data of an electric motor and is shown in Figure \ref{fig:example-motor}. 
\lstopar{
We simulated the heating patterns of a direct current permanent magnet electric motor \cite{book:1107411} using its typical characteristic curve.
} 
\begin{figure*}[]
  	\centering
...
To conduct the experiment we generated two dataset: a training set and a test set both containing $200k$ observations. We built a model with 20 finest scale states on the training set using two attributes: angular velocity and temperature. The training dataset was then replayed and the finest scale states stored. We then used
...
\begin{figure}[h!]
	...
	\caption{Histograms of all the attributes in one of the summer states of the weather example shown in Figure \ref{fig:example-weather}. The histograms suggest a high correlation of temperature and rainfall \lstopar{ in the dataset}.}
