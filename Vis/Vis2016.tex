...
%% Abstract section.
\abstract{
This paper presents StreamStory, a system for the analysis of multivariate time series. It computes and visualizes a \lstopar{hierarchical} continuous time Markov chain model which captures the qualitative behavior of the \lstopar{systems' dynamics}\primoz{dynamics of the system}. It provides a number of helper services to help \primoz{a user}\lstopar{users} interpret their data including automatic state \lstopar{labeling}\primoz{naming}, rule extraction as well as a number of different views on the model. The system implements a number of options for \lstopar{users}\primoz{the user to} determine how the model is computed.  

A key feature \primoz{of the system }is that \lstopar{it }does not \lstopar{compute}\primoz{build} a single Markov chain model, but rather a hierarchy of models allowing \lstopar{users}\primoz{the user} to interactively find suitable scales for interpreting \primoz{the }data. \primoz{The system also}\lstopar{It} incorporates a novel approach to computing multiscale Markov chains\lstopar{ using two methods: one considering the spatial properties of the data and the other considering properties of the computed Markov chain}.\primoz{ We present two methods one based on the spatial properties of the data and one based on the properties of the computed Markov chain model.}

Once computed, the system is fully interactive. We present an analysis of four different datasets, coming from weather data (e.g. rainfall and temperature), traffic \primoz{monitoring }sensors, GPS traces and (simulated) data of a motor in operation. We show that our system enables us to interpret various patters in the data. Finally, the system has the ability to visualize real-time streaming data by mapping it to a previously computed model. It can then provide predictions and alarms for different behaviors.  
