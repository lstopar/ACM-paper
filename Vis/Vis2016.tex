...
%% Abstract section.
\abstract{
This paper presents StreamStory, a system for the analysis of multivariate time series. It computes and visualizes a continuous time Markov chain model which captures the qualitative behavior of the dynamics of the system. It provides a number of helper services to help a user interpret their data including automatic state naming, rule extraction as well as a number of different views on the model.  The system implements a number of options for the user to determine how the model is computed.  

A key feature of the system is that does not build a single Markov chain model, but rather a hierarchy of models allowing the user to interactively find suitable scales for interpreting the data. The system also incorporates a novel approach to computing multiscale Markov chains. We present two methods one based on the spatial properties of the data and one based on the properties of the computed Markov chain model.
