...
\newcommand{\primoz}[1]{\textcolor{green}{#1}}
...
%% Abstract section.
\abstract{
This paper presents StreamStory, a system for the analysis of multivariate time series. It computes and visualizes a hierarchical continuous time Markov chain model which captures the qualitative behavior of the systems' dynamics. It provides a number of helper services to help the user interpret their data including automatic state labeling, rule extraction as well as a number of different views on the model. The system implements a number of options for the user to determine how the model is computed.  

A key feature is that it does not compute a single Markov chain model, but rather a hierarchy of models allowing the user to interactively find suitable scales for interpreting the data. We also introduce two new approaches to computing multiscale Markov chains: one considering the spatial properties of the data and the other considering properties of the computed Markov chain.

Once computed, the system is fully interactive. We present an analysis of four different datasets, coming from weather data (e.g. rainfall and temperature), traffic monitoring sensors, GPS traces and (simulated) data of a motor in operation. We show that our system \error{enables us to interpret various patters [TODO of what] in the data}. Finally, the system has the ability to visualize real-time streaming data by mapping it to a previously computed model. It can then provide predictions and alarms for different behaviors.  
...
\keywords{Time series, Multi-scale, Summarization, Markov chains, Exploratory data analysis}
...
\input{multiscale_framework.tex}


\section{Realization}
\label{sec:multiscale-implementation}
\input{multiscale_implementation.tex}
