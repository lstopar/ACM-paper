...
At the finest scale, we use the partition function \lstopar{$f: \mathbb{R}^d \rightarrow I$} defined above, inducing the state space $I$ of the Markov chain $(X_t)_{t \ge 0}$. 
We then compute \lstopar{$f_s$} by aggregating elements of $I$ using a mapping $\phi_s: I \rightarrow I_s$ and representing \lstopar{$f_s$} as a composition $\lstopar{f_s} = \lstopar{f} \circ \phi_s$.
