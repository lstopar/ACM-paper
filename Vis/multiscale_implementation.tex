...
To visualize the different scales, we use the MDS coordinates computed at the initial level. When states are aggregated, the position of the new state is computed as the \primoz{weighted?} average of the states' positions. This is again followed by a repulsive scheme to ensure that the states do not overlap.